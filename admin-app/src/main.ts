type ViewName = 'users' | 'history' | 'alerts' | 'logout';
...
let currentView: ViewName = 'users';
...
function selectedUser(): ManagedUser | undefined {
  return managedUsers.find(user => user.id === selectedUserId);
}

function renderUsers(filter = '') {
  const list = getById<HTMLElement>('userList');
  const counter = getById<HTMLElement>('userCount');
  ...
  list.querySelectorAll<HTMLElement>('[data-user-id]').forEach(button => {
...
function renderUserDetail() {
  const summary = getById<HTMLElement>('summaryCard');
  const tbody = getById<HTMLElement>('recordTableBody');
  const user = selectedUser();
  if (!summary || !tbody || !user) return;
...
function renderHistory() {
  const container = getById<HTMLElement>('historyList');
  if (!container) return;
...
function renderAlerts() {
  const container = getById<HTMLElement>('alertList');
  const counter = getById<HTMLElement>('alertCount');
...
function updateHeader() {
  const title = getById<HTMLElement>('pageTitle');
  const subtitle = getById<HTMLElement>('pageSubtitle');
  if (!title || !subtitle) return;

  const textMap: Record<ViewName, { title: string; subtitle: string }> = {
...
function openView(nextView: ViewName) {
...
function setupEvents() {
  document.querySelectorAll<HTMLElement>('.nav-item').forEach(item => {
    item.addEventListener('click', () => {
      const nextView = item.dataset.view as ViewName | undefined;
      if (nextView) openView(nextView);
    });
  });

  getById<HTMLElement>('brandHome')?.addEventListener('click', () => openView('users'));

  getById<HTMLInputElement>('userSearch')?.addEventListener('input', event => {
    renderUsers((event.target as HTMLInputElement).value);
  });

  getById<HTMLElement>('cancelLogout')?.addEventListener('click', () => {
    openView('users');
  });

  getById<HTMLElement>('confirmLogout')?.addEventListener('click', () => {
    const message = getById<HTMLElement>('logoutMessage');
    if (message) message.textContent = 'モック画面のため、実際のログアウトAPIは未接続です。';
  });

  getById<HTMLElement>('menuButton')?.addEventListener('click', () => {
    getById<HTMLElement>('sidebar')?.classList.toggle('open');
  });
}
