...
'use strict';
// TypeScript用の型定義（実行時の挙動は変わらない）
type Mode = 'feature1' | 'feature2' | 'feature3' | null;
...
interface AppState {
  ...
  lastMorningGreeting: string | null;
...
type RecordAmountsInput = Pick<RecordEntry, 'count' | 'count170' | 'pickupCount' | 'otherIncome' | 'mode' | 'skipTaxInCalc'>;
...
function getInputById(id: string): HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement | null {
  return getById<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>(id);
}
...
/** 税込計算（10%加算、端数は四捨五入） */
function addTax(value: number): number {
...
/** レコードの税抜合計 */
function calcRecordTaxEx(rec: RecordAmountsInput): number {
  // 計算モードで配達単価が変わる
  // rec は「1日の記録オブジェクト」
  let base = 0;
...
/** レコードの税込合計 */
function calcRecordTaxIn(rec: RecordAmountsInput): number {
  // 税込側は機能1のみ10%加算が必要
...
function getTaxInDisplay(rec: RecordAmountsInput): string {
  return rec.skipTaxInCalc ? '計算しない' : yen(calcRecordTaxIn(rec));
}

/** 指定年月のレコード配列を返す */
function getMonthRecords(year: number, month: number): RecordEntry[] {
  // YYYY-MM の接頭辞でフィルタする
 ...
 */
function calcMonthlyTotals(year: number, month: number) {
  // 月内レコードを集めて合計値を作る
...
function syncFormFromSelectedDate() {
  ...
  const skipTaxInCalc = getById<HTMLInputElement>('skipTaxInCalc');
  if (skipTaxInCalc) skipTaxInCalc.checked = !!rec?.skipTaxInCalc;
...
function renderModeRadio() {
  ...
  const r1 = getById<HTMLInputElement>('modeFeature1');
  const r2 = getById<HTMLInputElement>('modeFeature2');
  const r3 = getById<HTMLInputElement>('modeFeature3');
...
function updatePreview() {
  // 入力値だけで仮計算（保存はしない）
  const previewEl = getById<HTMLElement>('calcPreview');
  if (!previewEl) return;
  ...
  const mockRec = {
    ...
    mode,
    skipTaxInCalc: getById<HTMLInputElement>('skipTaxInCalc')?.checked ?? false,
  };
...
function showMemoModal(rec: RecordEntry) {
  // その日の内訳をモーダルに表示する
  const ex  = calcRecordTaxEx(rec);
  const inc = calcRecordTaxIn(rec);
  const totalCount = rec.count + rec.count170;

  const memoTitle = getById<HTMLElement>('memoTitle');
  const memoBody = getById<HTMLElement>('memoBody');
  if (!memoTitle || !memoBody) return;
  memoTitle.textContent = `${rec.date} の記録`;

  // 1行分のHTMLを作る小さな関数
  const memoRow = (label: string, value: string, mod = '') => {
...
function showPickerModal() {
  ...
  const yearSel  = getById<HTMLSelectElement>('pickerYear');
  const monthSel = getById<HTMLSelectElement>('pickerMonth');
...
function setupEvents() {
  ...
  const bind = (id: string, event: string, handler: EventListenerOrEventListenerObject) => {
  ...
  bind('authSignIn', 'click', async () => {
    ...
    if (!id || !pass) {
      const authMessage = getById<HTMLElement>('authMessage');
      if (authMessage) authMessage.textContent = 'IDとパスワードを入力してください。';
      return;
    }
    ...
    if (res.ok) {
      ...
      const authMessage = getById<HTMLElement>('authMessage');
      if (authMessage) authMessage.textContent = 'ログインに失敗しました。';
    }
  });
  ...
  bind('workdaySelect', 'change', (e: Event) => {
  ...
  // ── 年間グラフ 年選択 ──
  bind('annualYear', 'change', (e: Event) => {
    const target = e.target as HTMLSelectElement | null;
    if (!target) return;
  ...
  bind('exportData', 'click', async () => {
    ...
    const area = getById<HTMLTextAreaElement>('importData');
  ...
  document.querySelectorAll('input[name="mode"]').forEach(radio => {
    radio.addEventListener('change', (e) => {
      // ラジオ選択はフォームの状態にだけ影響
      state.currentMode = (e.target as HTMLInputElement).value as Mode;
      saveState();
...
function handleStepperClick(e: Event) {
  // ステッパーの + / - ボタンの共通処理
  const btn = (e.target as HTMLElement | null)?.closest<HTMLElement>('.stepper-btn');
  if (!btn) return;
...
function openSaveConfirm(skipExistingCheck = false) {
  ...
  const row = (label: string, value: string) => `<div class="confirm-row">
    <span class="confirm-label">${label}</span>
    <span class="confirm-value">${value}</span>
  </div>`;

  const saveConfirmBody = getById<HTMLElement>('saveConfirmBody');
  if (!saveConfirmBody) return;
...
function fillPrevRecord() {
  ...
  const skipTaxInCalc = getById<HTMLInputElement>('skipTaxInCalc');
  if (skipTaxInCalc) skipTaxInCalc.checked = !!prev.skipTaxInCalc;
...
function goMonth(delta: number) {
  ...
  const wrapper = getById<HTMLElement>('calendarSwipe');
  if (!wrapper) return;
 ...
 */
function renderAnnualChart(year: number) {
  const container = getById<HTMLElement>('annualChart');
  if (!container) return;

  const data: Array<{ month: number; ex: number; inc: number }> = [];
  let max = 0;
  for (let m = 0; m < 12; m++) {
...
function setupSwipe() {
  ...
  let dragging = false;
  let width = 1;
  let activePointerId: number | 'touch' | null = null;
  ...
  const onStart = (clientX: number, clientY: number, pointerId: number | 'touch') => {
    // スワイプ開始時の処理
...
function parseInputInt(id: string) {
  // 空欄や負数は0にする
 ...
 */
function showMessage(text: string, type: 'error' | 'success', targetId: string) {
  // 画面に一時メッセージを出す
  const el = getById<HTMLElement>(targetId);
  if (!el) return;
  el.textContent = text;
...
function getNextWorkDate(fromDateStr: string) {
  // 翌日から「休日ではない日」を探す
...
let alertOkHandler: (() => void) | null = null;
let alertCancelHandler: (() => void) | null = null;

function showAlert(message: string, options: AlertOptions = {}) {
  const modal = getById<HTMLElement>('alertModal');
  const msgEl = getById<HTMLElement>('alertMessage');
  const okBtn = getById<HTMLButtonElement>('alertOk');
  const cancelBtn = getById<HTMLButtonElement>('alertCancel');
...
/** メッセージをクリアする */
function clearMessage(targetId: string) {
  // メッセージ表示を消す
  const el = getById<HTMLElement>(targetId);
  if (!el) return;
  el.textContent = '';
...
function setupAnnualYearOptions() {
  const select = getById<HTMLSelectElement>('annualYear');
  if (!select) return;
  select.innerHTML = '';
  for (let y = 2025; y <= 2030; y++) {
    const opt = document.createElement('option');
